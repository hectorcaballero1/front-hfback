...
export default function Corpus() {
  ...
  const processFile = async (file: File) => {
    ...
    const invalid = documentos.findIndex(
      d => typeof d.fuente !== 'string' || typeof d.texto !== 'string',
    );
    if (invalid !== -1) {
  ...
  return (
    <div>
      <h1 className="page-title">Corpus RAG</h1>
      <p style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 24 }}>
        Ingesta documentos al RAG del tenant{' '}
        <span className="mono" style={{ color: 'var(--accent)' }}>{activeTenant}</span>.
        Re-subir la misma <span className="mono">fuente</span> actualiza sin duplicar.
      </p>
      ...
      <div
        ...
        role="button"
        tabIndex={0}
        aria-label="Soltar JSON de documentos aquí o hacer clic para seleccionar"
      ...
      {log.length > 0 && (
        <div style={{ marginTop: 32 }}>
          <div className="section-title">Corpus enviado recientemente</div>
      ...
      {log.length === 0 && (
        <div className="empty-state" style={{ paddingTop: 32 }}>
          <div className="empty-state__title">Sin documentos enviados todavía.</div>
          <div className="empty-state__desc">Sube un JSON con documentos para ingestar el corpus del RAG.</div>
        </div>
      )}
    </div>
  );
